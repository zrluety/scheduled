...
class Session:
    def __init__(self, source, profile, scheduled, dest, username=None, develop=False):
        ...
        self.profiles = [os.path.join(PROFILES_ROOT, p) for p in os.listdir(PROFILES_ROOT)
        ...
        msg = 'Session created on {self.source} ({self._given_profile}) by {self.username}'.format(self=self)
    ...
    def run(self):
        ...
        for profile in self.profiles:
            ...
            if not df.empty:
                self._logger.info('The following profile was used for extraction: '
